...
namespace EasyITCenter.Controllers {
    ...
    public class WebPagesAdminGroupMenuListApi : ControllerBase {
        ...
        public async Task<string> GetWebGroupMenuList() {
            if (EasyITCenterContextExtension.IsAdmin()) {
                List<WebGroupMenuList> data;
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                    IsolationLevel = IsolationLevel.ReadUncommitted //with NO LOCK
                })) {
                    data = new EasyITCenterContext().WebGroupMenuLists
            ...
            else { return JsonSerializer.Serialize(new ResultMessage() { Status = DBResult.error.ToString(), RecordCount = 0, ErrorMessage = DbOperations.DBTranslate("YouDoesNotHaveRights") }); }
        }
        ...
        public async Task<string> InsertOrUpdateGroupWebMenuList([FromBody] WebSettingList1 record) {
            try {
                if (EasyITCenterContextExtension.IsAdmin()) {
                    ...
                    int result = 0;
                    if (RecId == 0) {
                        var data = new EasyITCenterContext().WebGroupMenuLists.Add(groupWebMenu);
                        result = await data.Context.SaveChangesAsync();
                    }
                    else {
                        var data = new EasyITCenterContext().WebGroupMenuLists.Update(groupWebMenu);
                        result = await data.Context.SaveChangesAsync();
                    }

                    if (result > 0) return JsonSerializer.Serialize(new ResultMessage() { InsertedId = groupWebMenu.Id, Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
