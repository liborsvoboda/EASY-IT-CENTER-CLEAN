        ...
        public IActionResult Authenticate([FromHeader] string Authorization) {
            ...
            try {
                if (HttpContext.Connection.RemoteIpAddress != null && user != null) {
                    ...
                    if (!string.IsNullOrWhiteSpace(clientIPAddr)) { ServerCoreDbOperations.WriteVisit(clientIPAddr, user.Id, username); }
                }
            } catch { }

            if (user == null)
                return BadRequest(new { message = ServerCoreDbOperations.DBTranslate("UsernameOrPasswordIncorrect", BackendServer.ServerSettings.ConfigServerLanguage) });
