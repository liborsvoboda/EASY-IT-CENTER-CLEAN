    ...
    public class SystemFailListApi : ControllerBase {
        ...
        public async Task<string> GetSystemFailList() {
            ...
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                ...
                data = new EASYDATACenterContext().SystemFailLists.ToList();
            }
        ...
        public async Task<string> GetSystemFailListByFilter(string filter) {
            ...
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                ...
                data = new EASYDATACenterContext().SystemFailLists.FromSqlRaw("SELECT * FROM SystemFailList WHERE 1=1 AND " + filter.Replace("+", " ")).AsNoTracking().ToList();
            }
        ...
        public async Task<string> GetSystemFailListKey(int id) {
            ...
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions {
                ...
                data = new EASYDATACenterContext().SystemFailLists.Where(a => a.Id == id).First();
            }
        ...
        public async Task<string> InsertSystemFailList([FromBody] SystemFailList record) {
            try {
                ...
                var data = new EASYDATACenterContext().SystemFailLists.Add(record);
        ...
        public async Task<string> UpdateSystemFailList([FromBody] SystemFailList record) {
            try {
                var data = new EASYDATACenterContext().SystemFailLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { InsertedId = record.Id, Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { Status = DBResult.error.ToString(), RecordCount = result, ErrorMessage = string.Empty });
        ...
        public async Task<string> DeleteSystemFailList(string id) {
            try {
                ...
                var data = new EASYDATACenterContext().SystemFailLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { InsertedId = record.Id, Status = DBResult.success.ToString(), RecordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { Status = DBResult.error.ToString(), RecordCount = result, ErrorMessage = string.Empty });
            } catch (Exception ex) {
