...
namespace EASYDATACenter {
    ...
    public class Startup {
        ...
        public void Configure(IApplicationBuilder app, IHostApplicationLifetime serverLifetime, IActionDescriptorCollectionProvider routerActionProvider, ILoggerFactory loggerFactory) {
            ...
            if (bool.Parse(DbOperations.GetServerParameterLists("ConfigServerStartupOnHttps").Value)) { app.UseForwardedHeaders(new ForwardedHeadersOptions { ForwardedHeaders = ForwardedHeaders.All });app.UseCertificateForwarding(); }
            ...
            app.Use(async (HttpContext context, Func<Task> next) => {
                ...
                if (!context.WebSockets.IsWebSocketRequest)
                {
                    ...
                    if (serverApiSecurity != null)
                    {
                        ...
                        {
                            if (serverApiSecurity.RedirectPathOnError?.Length == 0) { redirected = true; context.Request.Path = "/StatusPageService/401UnauthorizedPage"; await next(); }
                            else
                            {
            ...
            if (bool.Parse(DbOperations.GetServerParameterLists("WebBrowserContentEnabled").Value))
            { //Browsable Path Definitions
                ...
                Console.WriteLine("Folder Browsing: " + data.ObjectToJson());
                data.ForEach(path => {
                    try
                    {
                        app.UseFileServer(new FileServerOptions
                        {
                            ...
                            RequestPath = path.WebRootSubPath.StartsWith("/") ? path.WebRootSubPath : "/" + path.WebRootSubPath,
                            EnableDirectoryBrowsing = true
                        });
                        if (!string.IsNullOrWhiteSpace(path.AliasPath))
                        {
                            app.UseFileServer(new FileServerOptions
                            {
                                FileProvider = new PhysicalFileProvider(Path.Combine(SrvRuntime.WebRootPath + FileOperations.ConvertSystemFilePathFromUrl(path.WebRootSubPath))),
                                RequestPath = !string.IsNullOrWhiteSpace(path.AliasPath) ? (path.AliasPath.StartsWith("/") ? path.AliasPath : "/" + path.AliasPath) : "",
                                EnableDirectoryBrowsing = true
                            });
                        }
                    }
